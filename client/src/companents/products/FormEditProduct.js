...
const FormEditProduct = ({pid}) => {
    ...
    const fecthDataProduct = async(pid) => {
    ...
    const handleCreateProduct = async() => {
        if (data.title 
            ...
            && data.quanlity) {
                ...
                const response = await apis.updateProduct(pid, dataPass)
    ...
    return (
        ...
        >
            ...
            <button onClick={handleCreateProduct} className='py-2 px-4 bg-main rounded-md text-white font-bold'>Cập nhật sản phẩm</button>
        </div>
    )
}
